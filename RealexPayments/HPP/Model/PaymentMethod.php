...
//		    "HPP_CHALLENGE_REQUEST_INDICATOR" => isset($hppChallengeRequestOptions[$hppChallengeRequestPreference]) ? $hppChallengeRequestOptions[$hppChallengeRequestPreference] : "NO_PREFERENCE"
	    ...
	    $hppBillingFields    = [
		    ...
		    "HPP_BILLING_STATE"      => in_array($billingAddress->getCountryId(), ['US', 'CA']) ? $billingAddress->getRegionCode() : '',
	    ...
	    $hppShippingFields = [
		    ...
		    "HPP_SHIPPING_STATE"      => !$isOrderVirtual && $shippingAddress ? (in_array($shippingAddress->getCountryId(), ['US', 'CA']) ? $shippingAddress->getRegionCode() : '') : '',
		    "HPP_SHIPPING_POSTALCODE" => !$isOrderVirtual && $shippingAddress ? $shippingAddress->getPostcode() : '',
		    "HPP_SHIPPING_COUNTRY"    => !$isOrderVirtual && $shippingAddress ? $this->getCountryNumericCode($shippingAddress->getCountryId()) : ''
	    ];
    ...
    public function refund(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        if ($fields['RESULT'] != '00') {
            throw new \Magento\Framework\Exception\LocalizedException(
                __('The refund action failed. Gateway Response - Error '.$fields['RESULT'].': '.
                $fields['MESSAGE'])
            );
        }
        $payment->setTransactionId($fields['PASREF'])
